<script lang="ts">
  import { TrithemiusCipher } from '../../lib/ciphers';
  ...
  function encrypt(): void {
    encryptEncrypted = TrithemiusCipher.encrypt(encryptMessage, encryptKey);
  }

  function decrypt(): void {
    decryptMessage = TrithemiusCipher.decrypt(decryptEncrypted, decryptKey);
  }
</script>

<div class="container my-5">
  <h1 class="mb-4">Szyfr Tritemiusza</h1>
    ...
    <div class="card-body">
      ...
      <div class="mb-3">
        ...
        <select id="encrypt-key" class="form-select" bind:value={encryptKey}>
          {#each AlphabetUtils.alphabet as character}
            <option value={character}>{character.toUpperCase()}</option>
          {/each}
        </select>
      </div>
      <div class="mb-3">
